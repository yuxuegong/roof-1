$(function() {
	...
	var table = new ROOF.SelectableTable($('#mailuser_mailUser_table'));
	var page = new com.sinopec.PageBar.PageBar($('#mailuser_mailUser_search_form'));
	//默认选中第一条，并且加载相应的页签
	defaultFirst(table);
	$("#mailuser_mailUser_create_btn").click(
					function() {
						var ref = ROOF.Utils.openwindow(ROOF.Utils
								.projectName()
								+ "/mail/mailuserAction/create_page.action", "t_mail_user管理",
								690, 320);
						return false;
					});
	$("#mailuser_mailUser_import_btn").click(
			function() {
				var ref = ROOF.Utils.openwindow(ROOF.Utils
						.projectName()
						+ "/mail/mailuserAction/import_page.action", "t_mail_user管理",
						690, 320);
				return false;
			});
	$("#mailuser_mailUser_detail_btn").click(
			function() {
				...
				var ref = ROOF.Utils.openwindow(ROOF.Utils.projectName()
						+ "/mail/mailuserAction/detail_page.action?id=" + id  , "t_mail_user管理", 690, 320);
				return false;
			});
	$("#mailuser_mailUser_update_btn").click(
			function() {
				var trs = table.getSelectedTrNoClone();
				if (trs.length == 0 || trs.length > 1) {
					alert("请选择一行记录");
				...
				var id = trs[0].find(":input[name='id']").val();
				var ref = ROOF.Utils.openwindow(ROOF.Utils.projectName()
						+ "/mail/mailuserAction/update_page.action?id=" + id  , "t_mail_user管理", 690, 320);
				return false;
			});
	$("#mailuser_mailUser_delete_btn").click(
					function() {
					var trs = table.getSelectedTrNoClone();
					if (trs.length == 0 || trs.length > 1) {
						alert("请选择一行记录");
					...
					$.ajax({
					    async: false,
					    url : basePathConst+"/mail/mailuserAction/delete.action",
					    data: {"id":id},
					    type: 'post',
					    dataType: 'json',
					    cache: false,
...
function loadTag(){
	...
	$("div[name='tagdivs']").eq(tag_index).children().eq(0).load(function(){
	    var mainheight = $(this).contents().find(".wrap").height()+30;
...
//默认选择第一条，并加载页签
function defaultFirst(defaultTable){
	var defaultTrs = defaultTable.getSelectedTrNoClone();
	if (defaultTrs.length == 0 || defaultTrs.length > 1) {
		return false;
	}
	var defaultid = defaultTrs[0].find("input[type='checkbox']").val();
	//绑定点击的id到页面记录的keyid，记录当前点击的keyid
	key_id = defaultid;
	//load页签的iframe
	loadTag();
