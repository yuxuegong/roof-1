...
public class MybatisFileInfoService implements FileInfoService {
	...
	public FileInfo loadByName(String name) throws FileInfoNotFoundException {
		...
		List<FileInfo> fileInfos = (List<FileInfo>) daoSupport
				.selectForList("org.roof.fileupload.impl.dao.selectSimpleFileInfo", fileInfo);
		if (CollectionUtils.isEmpty(fileInfos)) {
			throw new FileInfoNotFoundException("fileinfo name:[" + fileInfo.getName() + "] not found");
		}
		if (fileInfos.size() > 1) {
			throw new FileInfoNotFoundException("fileinfo name:[" + fileInfo.getName() + "] 大于1条数据");
		}
	...
	public FileInfo createFileInfo(Map<String, Object> xdata) {
		...
		FileInfo fileInfo = new SimpleFileInfo();
		if (StringUtils.isNotEmpty(displayName) && StringUtils.contains(displayName, ".")) {
			fileInfo.setName(fileNameGenerator.getFileName() + FileUtils.getExtention(displayName));
		} else {
			fileInfo.setName(fileNameGenerator.getFileName());
		}
		fileInfo.setDisplayName(displayName);
		if (xdata != null && xdata.get("ContentType") != null) {
			fileInfo.setType(ObjectUtils.toString(xdata.get("ContentType")));
		} else if (StringUtils.contains(displayName, ".")) {
			fileInfo.setType(fileTypeGenerator.getFileType(displayName));
		}
		fileInfo.setFileSize(Long.parseLong(xdata.get("fileSize").toString()));
		return fileInfo;
	}
