...
$(function() {
	...
	//默认选中第一条，并且加载相应的页签
	defaultFirst(table);
	$("#${sysName}_${alias}_create_btn").click(
					function() {
						var ref = ROOF.Utils.openwindow(ROOF.Utils
								.projectName()
								+ "/${actionName}Action/create_page.action", "${tableDisplay}管理",
								690, 320);
						return false;
					});
	$("#${sysName}_${alias}_detail_btn").click(
			function() {
				...
				var ref = ROOF.Utils.openwindow(ROOF.Utils.projectName()
						+ "/${actionName}Action/detail_page.action?${key}=" + ${key} <#if (drdsId ??)> + "&${drdsId}=" + ${drdsId}</#if> , "${tableDisplay}管理", 690, 320);
				return false;
			});
	$("#${sysName}_${alias}_update_btn").click(
			function() {
				var trs = table.getSelectedTrNoClone();
				if (trs.length == 0 || trs.length > 1) {
					alert("请选择一行记录");
				...
				var ref = ROOF.Utils.openwindow(ROOF.Utils.projectName()
						+ "/${actionName}Action/update_page.action?${key}=" + ${key} <#if (drdsId ??)> + "&${drdsId}=" + ${drdsId}</#if> , "${tableDisplay}管理", 690, 320);
				return false;
			});
	$("#${sysName}_${alias}_delete_btn").click(
					function() {
					var trs = table.getSelectedTrNoClone();
					if (trs.length == 0 || trs.length > 1) {
						alert("请选择一行记录");
