...
public class ${alias?cap_first}Service implements I${alias?cap_first}Service {
	...
	private I${alias?cap_first}QueryDao ${alias}QueryDao;
	...
	public ${alias?cap_first}Vo load(${alias?cap_first} ${alias}){
		return (${alias?cap_first}Vo)${alias}QueryDao.reload(${alias});
	}
	
	public ${alias?cap_first}Vo selectForObject(${alias?cap_first} ${alias}){
		return (${alias?cap_first}Vo)${alias}QueryDao.selectForObject("select${alias?cap_first}",${alias});
	}
	
	public List<${alias?cap_first}Vo> selectForList(${alias?cap_first} ${alias}){
		return (List<${alias?cap_first}Vo>)${alias}QueryDao.selectForList("select${alias?cap_first}",${alias});
	}
	
	public Page page(Page page, ${alias?cap_first} ${alias}) {
		return ${alias}QueryDao.page(page, ${alias});
	}
	...
	@Autowired
	public void setI${alias?cap_first}QueryDao(
			@Qualifier("${alias}QueryDao") I${alias?cap_first}QueryDao  ${alias}QueryDao) {
		this.${alias}QueryDao = ${alias}QueryDao;
	}
