...
$(function() {
	...
	$("#${sysName}_${alias}_create_btn").click(function() {
		var ref = ROOF.Utils.openwindow(ROOF.Utils.projectName() + "/letv/gcr/${actionName}Action/create_page.action", "${tableDisplay}管理", width, height, true, true);
		return false;
	});
	$("#${sysName}_${alias}_detail_btn").click(function() {
		...
		var ref = ROOF.Utils.openwindow(ROOF.Utils.projectName() + "/letv/gcr/${actionName}Action/detail_page.action?${key}=" + ${key} <#if (drdsId ??)> + "&${drdsId}=" + ${drdsId}</#if> , "${tableDisplay}管理", width, height, true);
		return false;
	});
	$("#${sysName}_${alias}_update_btn").click(function() {
		...
		var ref = ROOF.Utils.openwindow(ROOF.Utils.projectName() + "/letv/gcr/${actionName}Action/update_page.action?${key}=" + ${key} <#if (drdsId ??)> + "&${drdsId}=" + ${drdsId}</#if> , "${tableDisplay}管理", width, height, true);
		return false;
	});
