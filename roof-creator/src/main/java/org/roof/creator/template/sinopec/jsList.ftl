...
$(function() {
	...
	$("#${sysName}_${alias}_detail_btn").click(
			function() {
				...
				var ref = ROOF.Utils.openwindow(ROOF.Utils.projectName()
						+ "/${actionName}Action/detail_page.action?${key}=" + ${key} <#if (drdsId ??)> + "&${drdsId}=" + ${drdsId}</#if> , "${tableDisplay}管理", width, height);
				return false;
			});
	$("#${sysName}_${alias}_update_btn").click(
			function() {
				...
				var ref = ROOF.Utils.openwindow(ROOF.Utils.projectName()
						+ "/${actionName}Action/update_page.action?${key}=" + ${key} <#if (drdsId ??)> + "&${drdsId}=" + ${drdsId}</#if> , "${tableDisplay}管理", width, height);
				return false;
			});
